...
def closed_loop(cell, embedded, prev_action, debug=False):
  ...
  if debug:
    with tf.control_dependencies([tf.compat.v1.assert_equal(
        tf.shape(input=nested.flatten(posterior)[0])[1], tf.shape(input=embedded)[1])]):
...
def open_loop(cell, embedded, prev_action, context=1, debug=False):
  ...
  if debug:
    with tf.control_dependencies([tf.compat.v1.assert_equal(
        tf.shape(input=nested.flatten(state)[0])[1], tf.shape(input=embedded)[1])]):
...
def planned(
  ...
  if debug:
    with tf.control_dependencies([tf.compat.v1.assert_equal(
        tf.shape(input=nested.flatten(state)[0])[1], context + length)]):
      state = nested.map(tf.identity, state)
